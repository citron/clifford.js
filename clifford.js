...
class BaseVector extends Vector {
    get normalIndex() { throw Error("virtual method call"); }
}
...
class EuclideanBaseVector extends NormalBaseVector {
    get normalIndex() { return 2 + this.index * 2; }
}
class AntiEuclideanBaseVector extends NormalBaseVector {
    get normalIndex() { return 2 + this.index * 2 + 1; }
}
class NullBaseVector extends BaseVector {
    get normalIndex() {
        if (this.name == 'ο') {
            ...
            throw new Error('unknown null base vector name');
        }
    }
}
...
class Multiplication extends BinaryInternalOperator {
    get operatorCharacter() { return '*'; }
    get classesWithLowerOperatorPrecedence() {
...
class Exponential extends BinaryInternalOperator {
    ...
    get classesWithLowerOperatorPrecedence() {
        return [ Addition, Subtraction,
            Multiplication, InnerProduct, OuterProduct
        ];
    }
    simplify() {
        if (this.left instanceof Fraction &&
...
const grammar = `
{
    let $clifford = require('/usr/local/src/clifford.js/clifford.js');
}
start
...
statement
    = left:Identifier __ '=' __  right:AdditiveExpression {
...
AdditiveExpression
    = head:MultiplicativeExpression
      ...
      {
          return tail.reduce(
              (left, element) => new element[1](left, element[3])
              , head
          );
      }
...
AdditiveOperator
  = "+" { return $clifford.Addition; }
  / "-" { return $clifford.Subtraction; }

MultiplicativeOperator
  = "/"  { return $clifford.Division; }
  / "*"  { return $clifford.Multiplication; }

UnaryOperator
  = "+" { return x => x; }
  / "-" {
      return x => 
      new $clifford.Multiplication(new $clifford.Int(-1), x);
  }
...
frac = decimal_point DecimalDigit+
...
UnicodeCombiningMark "unicode combining mark"
  = Mn
  / Mc
...
module.exports = {
    ...
    Multiplication, Division, Exponential, Involution, Reversion, Dual
}
